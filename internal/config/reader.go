...
func (r *reader) Merge(kvs ...*source.KeyValue) error {
	...
	for _, kv := range kvs {
		...
		if err := r.opts.decoder(kv, next); err != nil {
			fmt.Errorf("Failed to config decode error: %v key: %s value: %s", err, kv.Key, string(kv.Value))
			return err
		}
		if err := mergo.Map(&merged, convertMap(next), mergo.WithOverride); err != nil {
			fmt.Errorf("Failed to config merge error: %v key: %s value: %s", err, kv.Key, string(kv.Value))
			return err
		}
	}
	r.lock.Lock()
