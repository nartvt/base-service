...
func NewSource(prefixes ...string) source.Source {
	return &env{prefixes: prefixes}
}

func (e *env) Load() (kv []*source.KeyValue, err error) {
	return e.load(os.Environ()), nil
}

func (e *env) load(envs []string) []*source.KeyValue {
	var kv []*source.KeyValue
